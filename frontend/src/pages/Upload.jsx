...
export default function Upload() {
  const { token } = useAuth();
  const [files, setFiles] = useState([]); const [chosen, setChosen] = useState(null);
  const [msg, setMsg] = useState(''); const [err, setErr] = useState('');

  const refresh = async () => {
    setErr(''); try { setFiles(await listFiles(token)); } catch(e){ setErr('Kunde inte hämta filer'); }
  };
  useEffect(()=>{ refresh(); },[]);
  ...
  const onUpload = async (e) => {
    ...
    setMsg(''); setErr('');
    try { await uploadFile(token, chosen); setMsg('Uppladdad!'); setChosen(null); await refresh(); }
    catch(e){ setErr('Uppladdning misslyckades'); }
  };
